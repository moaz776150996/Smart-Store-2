...
async function fetchProductsFromSheet(): Promise<SheetResult> {
  if (isFetching) {
    return cachedResult || { success: true, products: [] };
  }
  isFetching = true;
  console.log("Fetching products from Google Sheets Apps Script...");
  try {
    ...
    if (response.ok && (contentType.includes('application/json') || text.trim().startsWith('[') || text.trim().startsWith('{'))) {
      try {
        ...
        return { success: false, error: "JSON Parse Error", details: e?.message || String(e) };
      }
    } else {
      console.log('Apps Script returned non-JSON/error response, content-type:', contentType, 'Status:', response.status);
    ...
    console.log('Failed to fetch from Google Sheet Apps Script:', err);
    return {
      success: false,
